...
export const ANALYSIS_SUGGESTIONS_VERSION = 1;

// v1 library: a small, opinionated set of analyses derived from early user
...
export const ANALYSIS_SUGGESTIONS: AnalysisSuggestion[] = [
  ...
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      "Search the conversation extractions' notableMoments for entries that hint at late discoveries, wrong premises, or unstated assumptions. For 3–5 of the strongest hits, read enough of the conversation to confirm what the missed assumption was and how it could have been declared up front. Summarize the patterns and recommend prompt boundaries that would have caught them.",
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      "List saved conversations. For each, note totalMessages (from clog_get) relative to the project's typical message count. Identify outliers — conversations far above the project's median. Read enough of each outlier to characterize what made it long. Suggest specific workflow improvements.",
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      "List saved conversations and filter to those with `extraction.outcome` of `abandoned` or `partial`. Group by project. For each project, list the conversations and their summaries. Highlight any patterns (e.g., the same feature attempted multiple times).",
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      "Aggregate `extraction.toolsUsed` across saved conversations. Report the top tools by frequency and any tools that show up in only a few conversations but might be more broadly useful. Note any conversations that used a notably different toolset than the user's norm.",
  },
  {
    id: "noise_patterns",
    name: "Noise patterns",
  ...
  },
  {
    ...
    audience: "team",
    suggestedPrompt:
      "List saved conversations with origin remote. Group by author. For each author, summarize average totalMessages, common outcomes, and dominant tools. Highlight authors whose pattern differs from the group median. Do not speculate on why; just surface the outliers so the user can investigate.",
  },
];
