...
export const ANALYSIS_SUGGESTIONS: AnalysisSuggestion[] = [
  {
    ...
    audience: "both",
    suggestedPrompt:
      `Use clog_list with a reasonable limit to gather saved conversations. ${LIST_PAGING_GUIDANCE} For a sample of them, call clog_get with head=2 to see the user's opening messages. Then look across the openings for patterns: missing constraints, unstated goals, ambiguous success criteria. Report 3–5 concrete suggestions the user could apply to future intro prompts, citing the conversation ids you drew them from.`,
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      `Use clog_list with a reasonable limit to inspect saved conversation extractions. ${LIST_PAGING_GUIDANCE} Search the extractions' notableMoments for entries that hint at late discoveries, wrong premises, or unstated assumptions. For 3–5 of the strongest hits, read enough of the conversation to confirm what the missed assumption was and how it could have been declared up front. Summarize the patterns and recommend prompt boundaries that would have caught them.`,
  },
  {
  ...
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      `List saved conversations with clog_list and a reasonable limit. ${LIST_PAGING_GUIDANCE} Filter to those with \`extraction.outcome\` of \`abandoned\` or \`partial\`. Group by project. For each project, list the conversations and their summaries. Highlight any patterns (e.g., the same feature attempted multiple times).`,
  },
  {
    ...
    audience: "both",
    suggestedPrompt:
      `Use clog_list with a reasonable limit to inspect saved conversation extractions. ${LIST_PAGING_GUIDANCE} Aggregate \`extraction.toolsUsed\` across saved conversations. Report the top tools by frequency and any tools that show up in only a few conversations but might be more broadly useful. Note any conversations that used a notably different toolset than the user's norm.`,
  },
  {
    id: "noise_patterns",
    name: "Noise patterns",
