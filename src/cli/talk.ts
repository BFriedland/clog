...
function buildSummarizePrompt(state: ClogState): string {
  return [
    ...
    "",
    "Do not summarize conversations with `summaryKind: \"curated\"`. Remote conversations are read-only and `clog_update` will reject them — filter them out with `origin: \"local\"` rather than discovering this at write time.",
  ]
    .filter(Boolean)
    .join("\n");
}
